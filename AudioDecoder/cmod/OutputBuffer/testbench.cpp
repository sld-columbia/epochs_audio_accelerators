...
SC_MODULE (Source) {
  ...
  void run() {
    ...
    // => 8 samples*16 channels
    int rotate_out_int[8*16] = {
...
//    sync1.reset_sync_out();  // start signal
    ...
    for (int i = 0; i < cfg_block_size_int; i++) {  // sample
      nvhls::nv_scvector<ac_fixed<32,16,true>, 16> rotate_out_reg;
      for (int j = 0; j < 16; j++) {                // channel
        ac_int<32,true> tmp_int = rotate_out_int[16*i + j];                    // [sample][channel]

        ac_fixed<32,16,true> tmp_fixed;
        tmp_fixed.set_slc(0, tmp_int);
        rotate_out_reg[j] = tmp_fixed;
      }

      rotate_out.Push(rotate_out_reg);
      cout << "@" << sc_time_stamp() << " Push rotate_out data " << endl;
    }
