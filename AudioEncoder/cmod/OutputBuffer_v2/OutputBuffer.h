...
//      for (int i = 0; i < (length >> 3); i++) {
      ...
      for (int i = 0; i < (length >> 4); i++) {  
        ...
        nvhls::nv_scvector<ac_fixed<32,16,true>, 16> 
            audio_out_reg[2];
        
        for (int j = 0; j < 2; j++) {
          audio_out_reg[j] = audio_out.Pop();
          wait();
        }
        //wait();
        //audio_out_reg[1] = audio_out.Pop();
        //wait();

        for (int j = 0; j < 16; j++) { // channel 
          ac_int<32,true> idx = j*(block_size>>1) + i;
          ac_int<64,true> data;
          // litle endian
          data.set_slc(0, audio_out_reg[0][j].slc<32>(0));
          data.set_slc(32, audio_out_reg[1][j].slc<32>(0));
          mem[idx] = data;
          wait();
        }
        ...
        for (int j = 0; j < 8; j++) { // litle endian 
          ...
          //ac_int<32,true> data1; dataㄅ.set_slc(audio_out_reg[2*j]);
